...
export default function Ticket({
  ...
  const artistName = artist?.name?.trim() || "Spotify Artist";
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <img className="absolute w-auto h-full" src={icon} alt="Track icon" />
          <img
        ...
        >
          ...
          >
            {selectedTrack?.list &&
              selectedTrack?.list.map((selectedTrackItem, index) => (
                <div
                  key={index}
                  className={cn("relative h-auto group justify-center flex text-white items-center opacity-80")}
                >
...
const NotAvatarUser = ({
  ...
  return (
    ...
    >
      <div>
        <p className="text-xl font-bold">{artistHashtag}</p>
        <span className="block text-sm font-normal w-max text-white/60">{artistName}</span>
      </div>
    </div>
  );
};
